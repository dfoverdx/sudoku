...
import { Puppet, readGame } from './';

type SudokuComDifficulties = 'easy' | 'medium' | 'hard' | 'expert';

const SudokuDotComPuppet: Puppet<SudokuComDifficulties> = {
    name: 'Sudoku.com',
    ...
    async load(page, difficulty) {
        ...
        const x = await page.waitForSelector('#cookies-consent .banner-close');
        if (await x.boundingBox()) {
            await x.click();
        }

        await clickTopLeftCell(page);
    },

    async newGame(page, wasPrev) {
        let wasPrevSuccess = false;
        if (wasPrev) {
            wasPrevSuccess = !!await awaitRetry(async () => {
                const btn = await page.waitForSelector('.button.button-play', { timeout: 100 });
                if (!await btn.boundingBox()) {
                    await Promise.wait(900);
                    // wait for visibility
                    return null;
                }
        ...
        if (!wasPrevSuccess) {
            await page.waitForSelector('.new-game-button');
            await page.evaluate(() => {
                $('.new-game-button').click();
            });

            await page.waitForSelector('.new-game-menu-new');
            await page.evaluate(() => {
                $('.new-game-menu-new').click();
            });
        }
    ...
    readGame(page) {
        return readGame(page, '.game-row', '.cell-value', getCellValue);
    },

    async setCellValue(page, coord, value) {
        async function waitCellCoord({ td = '', child = '', timeout = 30000 }) {
            const selector =
                `.game-table tr:nth-child(${coord[0] + 1}) > td${td}:nth-child(${coord[1] + 1}) ${child}`.trim();
            return await page.waitForSelector(selector, { timeout });
        }

        const cell = await waitCellCoord({ td: '.game-cell' });

        do {
            await cell!.click();
        } while (!await waitCellCoord({ td: '.cell-selected', timeout: 100 }).catch(() => {}));

        // @ts-ignore
        do {
            await page.keyboard.press(value.toString());
        } while (!await waitCellCoord({ child: 'svg', timeout: 100 }).catch(() => {}));
    }
};

async function getCellValue(cell: ElementHandle): Promise<string> {
    const svg = await cell.$('svg');

    if (!svg) {
        return '.';
    }

    const d = await svg.$eval('path', p => p.getAttribute('d'));

    switch (d) {
        case 'M8.954 30V3.545h-.267c-.738.41-6.706 4.655-7.71 5.311V5.883c.635-.41 6.767-4.758 7.977-5.476h2.789V30h-2.79z':
            return '1';

        case 'M.12 9.57C.16 4.462 4.057.791 9.41.791c5.209 0 9.187 3.568 9.187 8.224 0 3.076-1.415 5.475-6.275 10.664l-7.998 8.53v.247h15.012V31H.284v-1.969L10.62 17.854c4.122-4.43 5.168-6.193 5.168-8.736 0-3.302-2.81-5.865-6.44-5.865-3.814 0-6.46 2.584-6.5 6.316v.02H.12v-.02z':
            return '2';
        ...
        case 'M15.855 30v-6.686H.987v-2.563C3.633 16.281 7.283 10.6 14.563.366h4.02v20.426h4.43v2.522h-4.43V30h-2.728zM3.92 20.628v.184h11.935V3.052h-.184C10.03 10.744 7.099 15.338 3.92 20.629z':
            return '4';
        ...
        case 'M3.017 30L16.696 3.155V2.93H.29V.407h19.277v2.625L6.01 30z':
            return '7';
...
async function clickTopLeftCell(page: Page) {
    await awaitRetry(async () => {
        const cell = await page.waitForSelector(`.game-table tr:nth-child(1) > td:nth-child(1)`);
        await cell.click();
        return page.waitForSelector(`.game-table tr:nth-child(1) > td.cell-selected:nth-child(1)`, { timeout: 1000 });
    });
}

export default SudokuDotComPuppet;